...
2. **テキストの管理**：セリフやナレーションを順序立てて配置（ドラッグ&ドロップ対応、キャラクター割り当て機能）
3. **画像・キャラクター設定**：背景画像とキャラクター画像を統合管理（プレビュー機能付き）
4. **ヘッダー設定**：ゲーム情報、ステータスを設定
5. **プレビュー機能**：実際のゲーム画面のように確認
6. **データの保存**：作成したイベントをデータベースに保存
